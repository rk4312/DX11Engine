...
void Game::Init()
{
	...
	directionalLight1.Direction = XMFLOAT3(0, 0, 1);
	...
	CreateWICTextureFromFile(device.Get(), 
		context.Get(), 
		GetFullPathTo_Wide(L"../../Assets/Textures/Metal/bronze_albedo.png").c_str(), 
		nullptr, 
		metalTextureSRV.GetAddressOf());

	CreateWICTextureFromFile(device.Get(),
		context.Get(),
		GetFullPathTo_Wide(L"../../Assets/Textures/Metal/bronze_normals.png").c_str(),
		nullptr,
		metalNormalSRV.GetAddressOf());

	CreateWICTextureFromFile(device.Get(),
		context.Get(),
		GetFullPathTo_Wide(L"../../Assets/Textures/Metal/bronze_roughness.png").c_str(),
		nullptr,
		metalRoughnessSRV.GetAddressOf());

	CreateWICTextureFromFile(device.Get(),
		context.Get(),
		GetFullPathTo_Wide(L"../../Assets/Textures/Metal/bronze_metal.png").c_str(),
		nullptr,
		metalMetalnessSRV.GetAddressOf());
	...
	// Creating Materials
	metalMat = std::make_shared<Material>(white, 0.15f, vertexShader, pixelShader);
	metalMat->AddTextureSRV("Albedo", metalTextureSRV);
	...
	metalMat->AddSampler("BasicSampler", basicSampler);
...
void Game::CreateBasicGeometry()
{
	...
	gameEntitiesVector.push_back(GameEntity(meshVector[5], metalMat));
	gameEntitiesVector.push_back(GameEntity(meshVector[6], stoneMat));
	gameEntitiesVector.push_back(GameEntity(meshVector[2], metalMat));
	gameEntitiesVector.push_back(GameEntity(meshVector[0], stoneMat));
	gameEntitiesVector.push_back(GameEntity(meshVector[1], metalMat));
	gameEntitiesVector.push_back(GameEntity(meshVector[3], stoneMat));
	gameEntitiesVector.push_back(GameEntity(meshVector[4], metalMat));
}
